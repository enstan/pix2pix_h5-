...
//
  ...
  //})
    ...
    //create_editon 相当main函数，在此基础上调用其他相关的函数
...
function Editor(config) {
  this.config = config
  this.view = new View(this.config.name, 750, 1500)     //设置edges2cats-->canvas的大小
  this.buffers = []
...
document.addEventListener("touchmove", function(e) {
    console.log("m","e.screenX");

  mouse_pos = {x: e.touches[0].clientX, y: e.touches[0].clientY}
  update()
})
...
document.addEventListener("touchstart", function(e) {
  ...
  drag_start = {x: e.touches[0].clientX, y: e.touches[0].clientY}
  mouse_down = true
  mouse_pressed = true
  update()
})
